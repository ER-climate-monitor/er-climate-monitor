...
async function saveDetectionModel(model: Model<DetectionDocument>, sensorId: string, sensorName: string, unit: string, timestamp: number, longitude: number, latitude: number, value: number): Promise<DetectionDocument> {
...
async function getLastXDetections(model: Model<DetectionDocument>, sensorId: string, last: number): Promise<Array<Detection>> {
    ...
    return (await model.find({sensorId: sensorId})
        .sort({_id: -1})
        .limit(last))
        .map(detection => { 
